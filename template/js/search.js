...
function renderTable(forms, page = 1) {
	...
	if (0 == forms.length)
		...
		for (let i = 0; i < display_forms.length; ++i) {
			...
			const texts = [F.id + "-" + (F.lvc + 1), `Lv ${F.baseLv} + ` + F.plusLv, "", "", F.health, F.ap, Math.round(F.dps + Number.EPSILON), F.kb, F.range, numStrT(F.attackF), F.speed, numStr(1.5 * F.price), numStr(display_forms[i][0])];
			for (let j = 0; j < 13; ++j) {
