	...
	generate_cats() {
		...
		const cats = catTable.map(cat => {
			const info = {
				...
				egg1: cat.egg_1 || null,
				egg2: cat.egg_2 || null,
				evolReq: cat.evol_require,
				evol4Req: cat.evol4_require,
				talents: cat.talents,
				ver: cat.version ? parseInt(cat.version, 10) : null,
				orbs: cat.orb_count ? parseInt(cat.orb_count, 10) : null,
				xpCurve: cat.xp_curve,
				evolDesc: cat.evol_description,
				evol4Desc: cat.evol4_description,
				lvCurve: parseInt(cat.level_curve, 10),
				obtnStage: cat.obtain_stage || null,
				evolStage: cat.evol_stage || null,
				fandom: cat.name_fandom,
			};
			return {
				info,
				forms: [],
