...
import {dbGet, dbCount, dbDelete} from './common.mjs';
...
describe('stage.mjs', function () {
	...
	describe('getStageExtra', function () {
		...
		it('load specified keys', async function () {
			await Stage.loadStageData();
			const extra = await Stage.getStageExtra(['lmGrp', 'eName']);
			assert.isObject(extra.lmGrp);
			assert.isUndefined(extra.grpName);
			assert.isUndefined(extra.conditions);
			assert.isUndefined(extra.matDrops);
			assert.isUndefined(extra.resetModes);
			assert.isUndefined(extra.rars);
			assert.isUndefined(extra.eggs);
			assert.isArray(extra.eName);
			assert.isUndefined(extra.rewards);
		});
